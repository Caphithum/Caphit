...
def upload():
    if request.method == 'POST':
        ...
        # 处理图片（YOLO）
        modelpath = 'static/model/swim.pt'
...
def upload_video():
    if request.method == 'POST':
        ...
        frame_data = []  # 存储每帧的处理前后信息
        modelpath = 'static/model/swim.pt'  # YOLO模型路径
        ...
        for i, frame_idx in enumerate(frame_indices):
            ...
            if ret:
                ...
                #
                # frame_filename = f"{os.path.splitext(video_filename)[0]}_frame_{i + 1}.jpg"
                # original_path = os.path.join(frame_dir, frame_filename)
                # cv2.imwrite(original_path, frame)
                #
                # # YOLO处理帧
                # processed_filename = f"processed_{frame_filename}"
                # processed_path = os.path.join(worked_dir, processed_filename)
                # yolo(modelpath, original_path, processed_path, 'drowner')
                # 原始帧保存（裁剪后的）
                frame_filename = f"{os.path.splitext(video_filename)[0]}_frame_{i + 1}.jpg"
                original_path = os.path.join(frame_dir, frame_filename)
                cv2.imwrite(original_path, cropped_frame)
                ...
                # 收集帧数据
                frame_data.append({
                    # 'original': url_for('static', filename=f'videos_img/{frame_filename}'),
                    'processed': url_for('static', filename=f'worked_videos/{processed_filename}'),
                    'frame_num': i + 1
                })
...
def uploadfall():
    if request.method == 'POST':
        ...
        # 处理图片（YOLO）
        modelpath = 'static/model/fall.pt'
        save_path = os.path.join(worked_dir, filename)  # 处理后图片保存路径
...
def uploadfight():
    if request.method == 'POST':
        ...
        # 处理图片（YOLO）
        modelpath = 'static/model/fight.pt'
        save_path = os.path.join(worked_dir, filename)  # 处理后图片保存路径
...
@app.route('/upload_videofall', methods=['GET', 'POST'])
def upload_videofall():
    if request.method == 'POST':
        file = request.files['video']
        if file.filename == '':
            return "未选择文件", 400
        ...
        frame_data = []  # 存储每帧的处理前后信息
        modelpath = 'static/model/fall.pt'  # YOLO模型路径
        ...
        for i, frame_idx in enumerate(frame_indices):
            ...
            if ret:
                ...
                processed_filename = f"processed_{frame_filename}"
                processed_path = os.path.join(worked_dir, processed_filename)
                yolo(modelpath, original_path, processed_path, 'falldown')
        ...
        # 传递所有帧数据到模板
        return render_template('index2fall.html',
                             video_filename=video_filename,
                             frames=frame_data)

    return render_template('upload_videofall.html')

@app.route('/upload_videofight', methods=['GET', 'POST'])
def upload_videofight():
    if request.method == 'POST':
        file = request.files['video']
        if file.filename == '':
            return "未选择文件", 400
        ...
        frame_data = []  # 存储每帧的处理前后信息
        modelpath = 'static/model/fight.pt'  # YOLO模型路径
        ...
        for i, frame_idx in enumerate(frame_indices):
            ...
            if ret:
                ...
                processed_filename = f"processed_{frame_filename}"
                processed_path = os.path.join(worked_dir, processed_filename)
                yolo(modelpath, original_path, processed_path, 'fight')
        ...
        # 传递所有帧数据到模板
        return render_template('index2fight.html',
                             video_filename=video_filename,
                             frames=frame_data)

    return render_template('upload_videofight.html')

@app.route('/index2fall', methods=['GET', 'POST'])
def index2fall():
    return render_template('index2fall.html')

@app.route('/index2fight', methods=['GET', 'POST'])
def index2fight():
    return render_template('index2fight.html')
